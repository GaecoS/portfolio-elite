...
if not df.empty:
    # A. TOP METRICS
    tot_cap, oggi_eu, tot_pl = df['Valore'].sum(), df['Oggi €'].sum(), df['Totale €'].sum()
    bg_o, tx_o = ("#064e3b", "#10b981") if oggi_eu >= 0 else ("#450a0a", "#ef4444")
    
    st.markdown(f"""
    <div style="display: flex; gap: 6px; width: 100%; height: 130px; margin-bottom: 20px;">
        <div style="flex: 1; background: #1e293b; border-radius: 10px; border-bottom: 4px solid white; display: flex; flex-direction: column; justify-content: center; align-items: center;">
            <p style="color: #94a3b8; margin: 0; font-size: 0.7rem; text-transform: uppercase;">Capitale</p>
            <h2 style="color: white; margin: 0; font-size: 1.8rem; font-weight: 800;">{format_no_dec(tot_cap)}</h2>
        </div>
        <div style="flex: 1; background: {bg_o}; border-radius: 10px; border-bottom: 4px solid {tx_o}; display: flex; flex-direction: column; justify-content: center; align-items: center;">
            <p style="color: #94a3b8; margin: 0; font-size: 0.7rem; text-transform: uppercase;">Oggi</p>
            <h2 style="color: {tx_o}; margin: 0; font-size: 1.8rem; font-weight: 800;">{format_no_dec(oggi_eu)}</h2>
        </div>
        <div style="flex: 1; background: #1e293b; border-radius: 10px; border-bottom: 4px solid #3b82f6; display: flex; flex-direction: column; justify-content: center; align-items: center;">
            <p style="color: #94a3b8; margin: 0; font-size: 0.7rem; text-transform: uppercase;">Profitto</p>
            <h2 style="color: #3b82f6; margin: 0; font-size: 1.8rem; font-weight: 800;">{format_no_dec(tot_pl)}</h2>
        </div>
    </div>
    ...
    # D. ANALISI STRATEGICA (SIMMETRIA TOTALE E COLORI SINCRONIZZATI)
    st.divider()
    ...
    # Costruzione Gradient
    parts = []; curr = 0
    for i, (s,v) in enumerate(sec_d.items()):
        parts.append(f"{colors[i % len(colors)]} {curr:.1f}% {curr+v:.1f}%")
        curr += v
    
    # Costruzione Legenda con colori sincronizzati e protezione overflow
    legenda_items = []
    for i, (s, v) in enumerate(sec_d.items()):
        item = f'''<div style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis; margin-bottom: 2px;">
                    <span style="color:{colors[i % len(colors)]}; font-size: 1rem;">●</span> 
                    <span style="font-size: 0.75rem;">{s}:</span> <b>{v:.0f}%</b>
                   </div>'''
        legenda_items.append(item)
    ...
    st.markdown(f'''
        <div style="display: flex; gap: 10px; width: 100%; flex-wrap: nowrap; align-items: stretch; margin-bottom: 20px;">
            ...
            <div style="flex: 1; display: flex; flex-direction: column; gap: 8px; height: 180px;">
                <div style="flex: 1; background: #1e293b; border-left: 8px solid #10b981; border-radius: 12px; padding: 12px 15px; display: flex; align-items: center; justify-content: space-between; box-sizing: border-box;">
                ...
                <div style="flex: 1; background: #1e293b; border-left: 8px solid #ef4444; border-radius: 12px; padding: 12px 15px; display: flex; align-items: center; justify-content: space-between; box-sizing: border-box;">
    ...
    st.markdown(f'''
        <div style="background: linear-gradient(90deg, #422006, #1e293b); padding: 25px; border-radius: 12px; border-left: 10px solid #f59e0b; display: flex; justify-content: space-around; align-items: center;">
            <div><p style="color:#f59e0b; margin:0; font-size:0.8rem; font-weight:bold;">CAPITALE AL TARGET</p><h2 style="color:#fbbf24; margin:0; font-size:2.5rem; font-weight:900;">{format_no_dec(cap_t)}</h2></div>
            <div style="width:2px; height:50px; background:#4b5563;"></div>
            <div><p style="color:#94a3b8; margin:0; font-size:0.8rem; font-weight:bold;">PLUSVALENZA ATTESA</p><h2 style="color:#10b981; margin:0; font-size:2.5rem; font-weight:900;">+ {format_no_dec(cap_t - tot_cap)}</h2></div>
        </div>
    ''', unsafe_allow_html=True)
